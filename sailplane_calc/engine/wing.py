...
def compute_effective_wing(panels: list[WingPanelInput]) -> EffectiveWingResult:
    """Dihedral-projected (horizontal-plane) span/area/AR: a panel's surface span is a slant
    ...
    if raw_eff_span == 0:
        return EffectiveWingResult(0.0, 0.0, 0.0, 0.0)
    ...
    return EffectiveWingResult(
        total_span=total_span, total_area=total_area, mean_chord=mean_chord, aspect_ratio=aspect_ratio
    )
...
def compute_wing(wing_input: WingInput) -> WingResult:
    ...
    effective = compute_effective_wing(wing_input.panels)
    return WingResult(
        surface=surface,
